...
if "started" not in st.session_state:
    st.session_state.update({
        "started": False, "secret": 0, "turns": 0, "max": 50, 
        "factor": 10, "over": False, "hint": "", "score": 0, "won": False, "level_name": ""
    })
...
with tab1:
    if not st.session_state.started:
        ...
        if st.button("Bắt đầu chơi"):
            ...
            st.session_state.update({
                "started": True, "secret": random.randint(1, limit), "turns": turns, 
                "max": limit, "factor": factor, "over": False, "hint": "", "score": 0, "won": False, "level_name": level.split()[0]
            })
            st.rerun()
    
    # TRƯỜNG HỢP ĐÃ CHIẾN THẮNG: Hiển thị màn hình ghi danh riêng biệt (Đã FIX ở đây)
    elif st.session_state.won:
        st.success(f"🎉 Chúc mừng! Bạn đã đoán đúng số bí mật là **{st.session_state.secret}**.")
        st.balloons()
        st.write(f"🏆 Bạn giành được: **{st.session_state.score} điểm**!")
        
        # Form nhập tên độc lập, không bị mất trạng thái khi bấm nút
        ten_nguoi_choi = st.text_input("Nhập tên của bạn để ghi danh BXH:", "Người chơi ẩn danh")
        if st.button("Lưu điểm số và Ghi danh"):
            luu_diem(ten_nguoi_choi, st.session_state.score, st.session_state.level_name)
            st.toast("Ghi danh thành công! Hãy qua tab BXH để xem.", icon="✅")
            st.session_state.won = False # Tắt màn hình ghi danh sau khi lưu
            st.rerun()
    ...
    # TRƯỜNG HỢP ĐANG CHƠI HOẶC THUA
    else:
        ...
        if st.button("Đoán số", disabled=(st.session_state.over or not valid)):
            ...
            if guess == st.session_state.secret:
                st.session_state.score = (st.session_state.turns + 1) * st.session_state.factor
...
with tab2:
    ...
    du_lieu_bxh = doc_bxh()
    if du_lieu_bxh:
        # Hiển thị BXH dạng bảng đẹp mắt
        st.table(du_lieu_bxh)
    else:
